...
- **Auth**: content endpoints require the existing `AuthGuard` (JWT); the whole surface is dev-gated by `KnowledgeWorkspaceGuard` (`KNOWLEDGE_WORKSPACE_ENABLED`; off in production unless explicitly enabled, returns 404 when off). The SSE stream is dev-gated only — `EventSource` can't send a bearer header — and emits only change metadata (path/type/title/timestamp), never content; the client re-fetches content through the authed endpoints.
