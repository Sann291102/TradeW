...
export default defineConfig({
  test: {
    include: [
      // ── Autonomous user-strategy agents ──────────────────────────────
      // Faithful ports of the sentinel-py indicator primitives. Every
      // difference from the Python original is a bug, so these are pinned
      // numerically rather than by shape.
      'src/user-strategy/indicators.spec.ts',
      // The canonical candle policy: closed bars only, each evaluated once.
      'src/user-strategy/candle-policy.spec.ts',
      // The deny-by-default tradability gate. Its most important assertions
      // are the ones about what it REFUSES to certify.
      'src/user-strategy/certification.spec.ts',
      // entry / waiting / refused — and why collapsing the last two is unsafe.
      'src/user-strategy/evaluate.spec.ts',
      // PHASE 4 — cross-engine parity against a corpus generated by the real
      // Python evaluator. This is the evidence behind "verified" in the
      // certification gate; without it that word means nothing.
