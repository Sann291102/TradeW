...
> **A third consumer, and the one thing Sentinel does that is not user-facing.** Since 2026-08-18 (completed 2026-08-30), `services/sentinel` also serves a service-token-only `POST /execution/evaluate` used by `services/api`'s **autonomous paper agents**. Everything in this blueprint still holds for the trader-facing path — it is literally the same orchestrator read (`observeInternal`), and Sentinel still never blocks, delays or instructs a human. What that endpoint adds is four gates (data quality, agent-strategy roster, index direction, evidence) and a **strike selection**, which lives here deliberately *because* it must never appear near a trader: ranking one contract out of three is exactly what `contract-alignment.ts` forbids in a user-facing surface. Its output goes to a machine account's PAPER order book, never to a person's screen. See [`AUTONOMOUS-PAPER-AGENTS.md`](AUTONOMOUS-PAPER-AGENTS.md).
