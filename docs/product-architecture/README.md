...
> **The one carve-out, and why it does not weaken that rule.** `services/api`'s **autonomous paper agents** ([`AUTONOMOUS-PAPER-AGENTS.md`](AUTONOMOUS-PAPER-AGENTS.md)) do open and manage positions without a human. They still leave the rule intact in the sense that matters: they never *execute on a user's instruction or in place of a user's judgment* — they trade only their own PAPER account, `ExecutionEnvironment` has exactly one member so real money is unrepresentable, and their output never reaches a human as advice. `services/sentinel` remains the analyst: it evaluates and returns a verdict, and it still holds no Prisma binding to `Order`/`Trade`/`Position`. The order is placed by `services/api`, where every order in this platform is placed.
...
3. `docs/product-architecture/TRADEW-AI.md` + `TRADEW-ASSISTANT.md` — full agent roster, feature set, workflows for Research, plus the voice/navigation/app-control assistant and its Sentinel auto-invocation. Then `AI-PERSONA.md`, `AI-VOICE.md` and `AI-CONVERSATION-LIFECYCLE.md` — the AI-shell layer that sits on top of them: the user-chosen name, the wake word and duplex voice, and the trading-day conversation that persists behind both.
...
9. `docs/product-architecture/AUTONOMOUS-PAPER-AGENTS.md` — **implemented.** The only order-placing agents in the platform, and the boundary that keeps them paper-only. Read it before touching `services/api/src/paper-execution/`, `services/sentinel/src/execution/`, or any `Execution*` model.
10. `../../ARCHITECTURE.md` — how these map onto services, communication patterns, and the dependency graph.
