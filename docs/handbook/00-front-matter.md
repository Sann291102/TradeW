...
2. **No AI-initiated trades.** Neither `services/tradew-ai` nor `services/sentinel` can reach the order path. Not gated, not with an override, not "just for the backtest." The dependency graph in §5.8 has no such arrow, and adding one is a rejected change, not a design discussion.
