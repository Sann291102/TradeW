...
import type { GraphData } from '@/lib/knowledge';
        ...
        {edges.map((e, i) => {
          ...
          return (
            <line key={i} x1={a.x} y1={a.y} x2={b.x} y2={b.y}
              stroke={lit ? '#14b8a6' : '#2a3a5c'} strokeWidth={lit ? 1.5 : 0.8}
        ...
        {nodes.map((n) => {
          ...
          return (
            <g key={n.id} transform={`translate(${p.x},${p.y})`} className="cursor-pointer"
              opacity={dim(n.id)} onMouseEnter={() => setHover(n.id)}
              onMouseDown={() => (dragRef.current = n.id)} onClick={() => onOpen(n.id)}>
