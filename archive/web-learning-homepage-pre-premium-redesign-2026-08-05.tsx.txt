 ...
 * (verbatim except the import paths, moved into `@/lib/learning-platform/*`
 * so the old Course/Lesson/Flashcard/Quiz model doesn't collide with the
 * catalog-based Strategy/Lesson model that superseded it — see
 * archive/README.md's 2026-08-04 entry for the original archiving decision,
 * and this component's restoration for why it came back as the /learning
 * homepage.
 *
 * Learning Hub landing — driven entirely by services/api's LearningController,
 * which proxies services/sentinel's learning-platform generation (still live,
 * unmodified since the frontend was archived).
 */
    ...
    <div className="mx-auto max-w-[1200px] space-y-4 p-4">
      ...
      <div>
        <div className="mb-2 flex items-baseline justify-between">
          ...
          <span className="text-[11px] text-faint">the previous catalog-based Strategies list</span>
        </div>
        <Link
          href="/learning/strategies"
          className="block rounded-card border border-border bg-card p-4 shadow-card transition-colors duration-micro hover:border-teal"
        >
